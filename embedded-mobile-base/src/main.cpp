...
void update_led_based_on_velocity(double velocity_left, double velocity_right) {
  ...
  double avg_vel = (velocity_left + velocity_right) /
                   2.0; // Average velocity (forward/backward)
  double diff_vel =
      velocity_left - velocity_right; // Velocity difference (turning)
...
void execute_commands_and_update_robot_state_with_physics() {
  ...
  // Safety check for time step - ensure positive and reasonable
  if (dt <= 0.001 || dt > 0.2) { // Minimum 1ms to prevent division issues
  ...
  for (int i = 0; i < num_joints; ++i) {
    // Dead zone: if commanded velocity is near zero AND current velocity is
    // small, stop completely
    ...
    double control_force = 5.0 * velocity_error;
    ...
    // Ensure inertia is never zero or negative to prevent division by zero
    if (joint_inertia[i] <= 0.001) {
      joint_inertia[i] = 0.05; // Reset to safe default
    }

    // Calculate acceleration from force and inertia
    double acceleration = total_force / joint_inertia[i];

    // Bound acceleration to prevent numerical runaway
    ...
    // Bound velocity to reasonable range
    if (current_velocity[i] > 5.0)
      current_velocity[i] = 5.0;
    if (current_velocity[i] < -5.0)
      current_velocity[i] = -5.0;
    ...
    // Normalize the position to a range of [-pi, pi] with bounds check
    if (abs(desired_position[i]) <
        1000.0) { // Only normalize if reasonable value
      ...
      // Reset to 0 if position has gone completely off the rails
...
void joint_state_callback(uint8_t *rx_data, size_t data_len) {
  ...
  // Create the JointState message and initialize it with the desired array
  // sizes ros2_control HW interface is only sending each joint a velocity
  // command
  ros_JointState js = {
...
void publish_joint_state() {
  ...
  static const char *const names[] = {"wbot_wheel_left_joint",
                                      "wbot_wheel_right_joint",
                                      "arbitrary_made_up_joint"};
...
void setup(void) {
  ...
  Serial.begin(115200);
  do { // blink the LED Orange to signal start of the program
  ...
  while (
      WiFi.status() !=
      WL_CONNECTED) { // blink the LED Blue to signal we are connecting to WiFi
...
void loop() {
  ...
  // blinkRGB(0, 100, 0, 10); // sleep for 10 ms and blink the LED Green to let
  // the user know the message went out
  delay(100);
}
