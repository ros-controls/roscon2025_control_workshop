...
void execute_commands_and_update_robot_state_with_physics() {
    ...
    for (int i = 0; i < num_joints; ++i) {
        ...
        if (current_velocity[i] > 5.0) current_velocity[i] = 5.0;
        if (current_velocity[i] < -5.0) current_velocity[i] = -5.0;
        ...
        if (abs(desired_position[i]) < 1000.0) {  // Only normalize if reasonable value
...
void joint_state_callback(uint8_t* rx_data, size_t data_len){
    ...
    if (ps_deserialize(rx_data, &js, data_len)){
        ...
        // execute_commands_and_update_robot_state_with_physics();
