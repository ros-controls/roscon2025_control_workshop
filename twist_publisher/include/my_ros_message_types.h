 ...
 * BTYPE = Basic type - only one member, implemented as typedef. FUNC(name, rmw_name, rmw_hash,
 * type) TTYPE = Typedef type - alias for ros type, implemented as typedef. FUNC(name, rmw_name,
 * rmw_hash, type) (separated to disable types in _Generic calls) CTYPE = Compound type - more
 * members, implemented as struct. FUNC(name, rmw_name, rmw_hash, <fields...>) FIELD = Field of
 * compound type FUNC(type, name) ARRAY = Array field of compound type FUNC(type, name, size)
 *
...
#define MSG_LIST(BTYPE, CTYPE, TTYPE, FIELD, ARRAY)                                              \
  BTYPE(                                                                                         \
    ros_Int32, "example_interfaces::msg::dds_::Int32",                                           \
    "5cd04cd7f3adb9d6c6064c316047b24c76622eb89144f300b536d657fd55e652", int32_t)                 \
  BTYPE(                                                                                         \
    ros_Int64, "example_interfaces::msg::dds_::Int64",                                           \
    "1b3b9a6502f560d079520c73c685a9550e5a1838d2cefd537fe0aba75a3639a0", int64_t)                 \
  BTYPE(                                                                                         \
  ...
  TTYPE(                                                                                         \
    ...
    "6963084842a9b04494d6b2941d11444708d892da2f4b09843b9c43f42a7f6881", ros_Vector3)             \
  CTYPE(                                                                                         \
    ros_Time, "builtin_interfaces::msg::dds_::Time",                                             \
    "b106235e25a4c5ed35098aa0a61a3ee9c9b18d197f398b0e4206cea9acf9c197",                          \
    FIELD(int32_t, sec) FIELD(int32_t, nsec))                                                    \
  CTYPE(                                                                                         \
    ros_Header, "builtin_interfaces::msg::dds_::Header",                                         \
    "f49fb3ae2cf070f793645ff749683ac6b06203e41c891e17701b1cb597ce6a01",                          \
    FIELD(ros_Time, time) FIELD(rstring, frame_id))                                              \
  CTYPE(                                                                                         \
 ...
 * SRV = Top level service name, hash and type. FUNC(srv_name, rmw_name, rmw_hash, <request>,
 * <reply>) REQUEST / REPLY = Request/reply type, implemented as struct. FUNC(<fields...>) FIELD =
 * Field of request/reply member. FUNC(type, name) ARRAY = Array field of request/reply member.
 * FUNC(type, name, size)
 *
 * Each entry in table must have the following format
 * SRV(                  \
...
#define SRV_LIST(SRV, REQUEST, REPLY, FIELD, ARRAY)                     \
  SRV(                                                                  \
    srv_add2Ints, "example_interfaces::srv::dds_::AddTwoInts",          \
    "e118de6bf5eeb66a2491b5bda11202e7b68f198d6f67922cf30364858239c81a", \
    REQUEST(FIELD(ros_Int64, a) FIELD(ros_Int64, b)), REPLY(FIELD(ros_Int64, sum)))
